...
대가는 지연이다. Gemini Live가 0.5초, 이쪽은 **실물 로봇에서 중앙값 약 2.0초**다. §성능 참고.
...
캐스케이드는 타협이 아니라 유일한 길이다. **양자화 — 로드맵에서 뺐다.** 서빙 가능한 E4B q4
체크포인트가 사실상 없고, 무엇보다 양자화가 무너뜨리는 능력(멀티모달 인코더, 정밀한 토큰
지시 따르기)이 **이 프로젝트가 서 있는 바로 그 능력**이다. 얻는 건 1초, 거는 건 운율 인지다.
...
PYTHONPATH= .venv_tts/bin/python client/test_tool_schemas.py  # 툴 유효값이 모델까지 가는가 (서버 불필요)
PYTHONPATH= .venv_tts/bin/python client/test_error_path.py    # 실패한 턴이 턴을 끝내는가
PYTHONPATH= .venv_tts/bin/python client/test_injected_turn.py # 주입 턴이 사용자 발화로 안 돌아오는가
...
| | |
|---|---|
| 말 멈춤 → 첫 소리 | **실물 로봇 중앙값 약 2.0초** (마이크·스피커 경유, 2026-09-23) |
| 끼어들기 반응 | **0.10초** |
| 첫 턴 (콜드 페르소나) | 17.9초 → 예열하면 **0.2초** |
| 디코딩 | 13.2 tok/s (메모리 대역폭 바운드: 204GB/s ÷ 15GB ≈ 13.6) |
| warm TTFT | 0.209초 |
| 발열·전력 | GPU 0~99%, tj **51~59°C**, RAM 37.3~37.6GB (실사용 131샘플, 스로틀링 없음) |

⚠️ **지연은 점이 아니라 범위로 읽어야 한다.** 뇌 안 루프백으로 같은 조건 4회를 재면
2.65 / 2.53 / 4.73 / 3.63초로 갈린다 — 첫 오디오가 **첫 문장**을 기다리는데 그 길이가
샘플링에서 흔들리기 때문이다. **n=8로는 2초 미만의 변화를 판별할 수 없다.**

실물 로봇 값(중앙값 약 2.0초)이 루프백보다 **오히려 낫다.** 루프백 하네스가 꼬리 침묵을
실시간으로 흘려보내는데 클립 자체에도 침묵이 들어 있어 대기가 부풀려지기 때문이다.
...
**못 따라잡은 것은 둘이다.**
- **지연** — 0.5초 대 약 2.0초. 남은 지렛대는 프롬프트 한 줄(첫 문장을 짧게)뿐이고,
  그 뒤는 13 tok/s가 하드웨어 바닥이다
...
않는다. **반복해서** 겪었고 — parity matrix의 세 행, 프롬프트 순서의 근거, 이미 해결된 걸
"미해결"로 둔 체크박스, "배선 안 됨"이라고 이틀을 거짓말한 docstring — 전부 **나중 수정이
옳았기 때문에** 생겼다. 그래서 상태를 주장하는 표는 측정이 들어올 때마다 다시 읽는다.
(횟수를 적어두지 않는 이유도 같다. 횟수 자체가 낡는다.)

**테스트 조건이 프로덕션보다 쉬우면 코드 경로가 통째로 빠진다.** 숫자만 낙관적으로 나오는 게
아니다. 네 가지 얼굴로 물렸다:
- **너무 작게** — 벤치마크가 50토큰 페르소나에 **툴 0개**를 써서 툴 파싱 60줄이 한 번도 안
  돌았다. 실제 페르소나로 바꾸자 로봇이 소리내어 읽었을 마크업 4종이 쏟아졌다
- **너무 빠르게** — 가짜 로봇이 오디오를 소켓 속도로 받아 **실시간 재생이라는 개념이
  없었다.** "로봇이 말하는 중에 끼어들기" 창이 어떤 테스트에도 없었고, 실물에서 barge-in이
  한 번도 안 먹었다
- **너무 정돈되게** — 손으로 쓴 stand-in 툴이 한 줄 docstring에 `Literal` 힌트라서, 실제
  로봇이 쓰는 `Args:` 파싱 경로가 한 번도 안 돌았다. 정돈됨도 비현실의 한 종류다
- **아예 없게** — 주입 텍스트 턴은 프로덕션에만 있고 테스트가 한 번도 만들지 않은 경로였다.
  그래서 로봇 자신의 말이 사용자 발화로 대화록에 남았다
...
**모델 앞에 놓은 글은 출력으로 돌아올 수 있다.** 전사 경로에서는 그 출력이 사람의 말로
보관된다 — 로봇이 주입한 안내문과 우리 전사 프롬프트가 각각 한 번씩 "사용자가 한 말"로
연구 데이터에 들어갔다.
...
**Stage 0~4 완료** (2026-09-23). 실물 로봇에서 대화가 끝까지 돈다.

- ✅ 실시간 스트리밍, 턴 감지, 툴 왕복, 재연결 복구, 끼어들기, 맞장구, 능동적 침묵
- ✅ 실물 로봇 — 인사·툴(제스처 포함)·전사·대화록·정상 종료까지 완주.
  AEC 정상(PulseAudio `module-echo-cancel`), 실제 마이크 기준 첫 오디오 **중앙값 약 2.0초**
- ⬜ **EXP-1 — 품질 기준선.** 남은 실질 작업은 이것 하나다. *로컬 모델이 한국어 공감 대화에
  충분한가* 가 이 프로젝트의 실제 연구 질문이고, 아직 한 번도 재지 않았다.
  아직 안 밟아본 경로 둘(**볼륨 있는 상태의 끼어들기**, **20교환 초과 대화**)이 여기 얹혀
  따라온다 — 시나리오 30개면 긴 대화다
- ⬜ **Stage 6 Tailscale** — 지금은 같은 LAN만. `direct` 유지 여부가 관건(§18)
- ⬜ (선택) **CosyVoice 2** — Gemini 대비 남은 두 격차 중 "목소리의 감정"을 메울 유일한 길.
  Piper가 바닥을 받쳐주고 있어 리스크가 아니라 품질 개선 항목이다

**로컬 개발은 Stage 4에서 마무리한다.** EXP-1부터는 실험 설계·시나리오·평가 방식이 연구
판단이라 저자가 직접 세운다.
