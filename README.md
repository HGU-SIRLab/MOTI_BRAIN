...
대화가 주제다), 벤더가 모델을 바꿔도 **로봇 성격이 안 변하고**(재현성), 턴 감지 임계값 하나를
...
멱등하다. 이미 떠 있는 건 건드리지 않으므로 상태가 의심스러우면 그냥 돌려도 된다.
...
**측정이 벤더 가이드를 이긴다.** 추측한 숫자는 `[UNVERIFIED]`로 표시하고 사실처럼 쓰지 않는다.
