...
### ⚠️ 첫 인사는 14초 걸린다 (아직 안 고쳐짐)

뇌를 새로 띄운 뒤 **첫 번째 턴**은 페르소나 18K 프리필을 통째로 문다 — 실측 14.17초.
`launcher.py`는 연결하자마자 인사 턴을 밀어넣으므로, **사람이 앞에 섰을 때 로봇이 14초간
가만히 있는다.** 두 번째 턴부터는 정상(1.4~2.7초)이다.

스펙 §13.1이 해결책(뇌 기동 시 예열)을 적어두었지만 **구현돼 있지 않다.** 지금 할 수 있는
우회는 둘 중 하나다:
- 사람을 만나기 전에 아무 세션이나 한 번 붙였다 끊어서 캐시를 데워둔다
- 뇌와 vLLM을 계속 켜둔다 — 캐시는 vLLM 프로세스가 사는 동안 유지되므로 이 비용은
  **vLLM 재기동당 한 번**이다
